...
Paper 预览和 Reader 的块级笔记/高亮现在保存独立 `paper` / `reader` 作用域和原译文侧。同一段同一偏移可在两个视图分别保存；正文只显示本视图对应侧的记录，列表和 Markdown 导出明确标出来源，点击 Paper 记录会回到 Paper 原块、恢复 Bilingual 并精确重选文字，同时接入 Back / Forward。旧版没有 scope 的记录继续按 Reader 处理。Paper 本身也与 macOS 一样提供 Original / Bilingual / Translation 三种完整文档模式，非 MinerU 预览不再在第 12 块截断，因此长文后半部分及 Paper 译文都可直接选择和标注。第三个工作区按 macOS 命名为 Overview，并把原先散落在 Paper 的阅读地图、提取质量 Review 与 AI 双语摘要合到同一页；内部仍接受旧 `Summary` 状态，已保存位置无需迁移。
