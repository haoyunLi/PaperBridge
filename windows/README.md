...
The current reader supports PDF drag/drop, automatic MinerU-first extraction with PDF text fallback, a separate new extraction copy, three reading modes, chapter translation with exact unfinished counts, source-checked bilingual summaries, exact Reader notes and highlights, and selection tools in Paper, Overview summaries, and Full Translation. Paper shows the exact native PDF in Original mode and the full reflowed document in Bilingual or Translation mode; Overview combines extraction review, an exact five-topic source map and optional bilingual summary like the macOS workspace. Its normalized heading rules never borrow an excerpt from the next section, and the guide includes the Mac questions, translation progress, Start Reading and View Original actions. Paper and Reader block annotations keep separate scopes and original/translation sides. Exact-PDF annotations created inside Paper are also separate from annotations in the extra Original tab. Every record renders only in its source view, carries an explicit export label, and returns to the original text and display mode; older unscoped block/PDF records remain Reader/Original annotations. When the inspector is closed, a macOS-style Quick Selection card offers translation, explanation, Amber/Cobalt/Coral highlighting, explicit highlight removal, terminology and Notes & More. Quick Selection and the expanded inspector both show lookup progress, Cancel, completion, cache-hit and local error messages. Translation and explanation stay visible together; choosing another color replaces the old color for that exact selection, while removing the highlight keeps its note. Under 1400px the Research Inspector uses the macOS-style 190–340px bottom drawer with separate scrolling columns for selection and supporting content; wider windows restore the right sidebar. Legacy `blue` records remain readable as Cobalt. Independent plain-text and Markdown headings use compact bilingual rows with translation/retry, source and translation annotations, bookmarks, explanation, structure actions and translated exports; a heading followed by body text in the same paragraph stays a normal translatable paragraph. Notes update immediately, coalesce disk writes for 350 ms, preserve exact whitespace, flush when editing ends or the app closes, and retain a retryable in-memory snapshot after save failure. Reading jumps keep up to 50 Back / Forward locations in the current paper session, including tab, display mode, search, block, PDF page and per-view scroll; use the header buttons, Ctrl+[ / Ctrl+], or the Paper menu. Each paper saves its languages, models, parsing choices, explanation language and results, and inspector state; reading appearance stays global. Output caches follow their settings and source, and the last opened paper is recorded separately from library modification order. Task and save state stay above the scrolling document, and cancelled requests that finish late are ignored.

Overview summaries and Full Translation display inline highlights and support saved-note navigation. Annotation undo preserves AI outputs generated afterward. Reader navigation clears search filters, and the current translation section follows scrolling. **Export portable Markdown bundle** writes Markdown files, image assets, the unchanged original PDF, and PNG reading copies for up to the first 120 PDF pages. Analysis and bilingual exports include bookmarks, highlights, notes, source/translation sides, and review notices for changed anchors. Page images can take time and disk space on long papers. See the mapping for the remaining details.
...
npm run test:heading-parity-e2e
